...
public struct PersonRepository: Sendable {
    ...
    public func create(
        ...
        try database.withStatement(
            "INSERT INTO people(id, group_id, name, attributes_json, created_at, updated_at) VALUES(?, ?, ?, ?, ?, ?);"
        ) { statement in
    ...
    public func list(groupId: String) throws -> [Person] {
        try database.withStatement(
            "SELECT id, group_id, name, attributes_json, created_at, updated_at FROM people WHERE group_id = ? ORDER BY updated_at DESC;"
        ) { statement in
    ...
    public func search(groupId: String, nameQuery: String) throws -> [Person] {
        let pattern = "%" + nameQuery + "%"
        return try database.withStatement(
            "SELECT id, group_id, name, attributes_json, created_at, updated_at FROM people WHERE group_id = ? AND name LIKE ? ORDER BY updated_at DESC;"
        ) { statement in
            statement.bindText(groupId, index: 1)
            ...
            while statement.step() == SQLiteStatement.row {
                ...
                let updatedAt = Date(timeIntervalSince1970: TimeInterval(statement.columnInt64(5)))
                let attributes = (try? SQLiteJSON.decodeDictionary(attributesJSON)) ?? [:]
                result.append(Person(id: id, groupId: groupId, name: name, attributes: attributes, createdAt: createdAt, updatedAt: updatedAt))
            }
            return result
        }
    }
    ...
    public func update(
        ...
        let attributesJSON = try SQLiteJSON.encodeDictionary(attributes)
        try database.withStatement(
            "UPDATE people SET name = ?, attributes_json = ?, updated_at = ? WHERE id = ?;"
        ) { statement in
            ...
            statement.bindText(personId, index: 4)
            let result = statement.step()
            guard result == SQLiteStatement.done else {
                throw SQLiteError(code: result, message: database.lastErrorMessage())
            }
    ...
    public func delete(personId: String) throws {
        try database.withStatement("DELETE FROM people WHERE id = ?;") { statement in
            statement.bindText(personId, index: 1)
            let result = statement.step()
            guard result == SQLiteStatement.done else {
                throw SQLiteError(code: result, message: database.lastErrorMessage())
            }
